@if ($solicitud->observaciones_vobo || $solicitud->observaciones)
    <div class="my-4">
        <p class="bg-red-100 text-red-500 font-bold py-1 px-2 rounded-sm border border-red-500">
            <svg class="inline-block w-5 h-5 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                viewBox="0 0 20 20">
                <path
            ...
            @if ($solicitud->observaciones_vobo)
                <span class="block pl-12 font-normal"><span class="font-bold">Por visto bueno:</span>
...
<div>
    ...
    <div class="mt-6">
        ...
        <select class="sm:w-auto w-full" title="Este campo no se puede modificar." id="id_rubro" name="id_rubro"
            wire:model="id_rubro" disabled>
            @foreach ($cuentasContables as $cuentaContable)
                <option value="{{ $cuentaContable->id }}" data-id-especial="{{ $cuentaContable->id_especial }}">
    ...
    <div class="mt-8" x-data x-init="tipoComprobacionOption = '{{ $tipo_comprobacion }}'">
        ...
        <input type="number" id="monto_total" name="monto_total" wire:model="monto_total"
            class="inputs-formulario-solicitudes w-40" title="Este campo no se puede modificar." min="0"
    ...
    <div class="mt-8">
        ...
        <input wire:model="concepto" class="inputs-formulario-solicitudes sm:w-[477px] w-full" id="concepto"
            type="text" placeholder="Concepto" title="Este campo no se puede modificar." disabled>
    </div>
    ...
    <div class="mt-8">
        ...
        <textarea wire:model="justificacionS" class="sm:w-3/4 w-full block" rows="3" cols="30" id="justificacionS"
            placeholder="Justificación" title="Este campo no se puede modificar." disabled></textarea>
    </div>
        ...
        <div class="mt-8">
            ...
            <div class="mt-2 sm:ml-10 sm:grid sm:grid-cols-2 gap-4 flex-col sm:w-3/4 w-full">
                <div class="flex-col">
                    ...
                    <input wire:model="finicial" class="inputs-formulario" title="Este campo no se puede modificar."
                        id="finicial" type="date" placeholder=""
                        min="{{ Carbon::now()->addDay(15)->format('Y-m-d') }}" disabled>
                </div>
                <div class="flex-col">
                    <label class="block mb-1 sm:mt-0 mt-4" for="ffinal">Fecha
                        final:</label>
                    <input wire:model="ffinal" class="inputs-formulario" title="Este campo no se puede modificar."
                        id="ffinal" type="date" placeholder=""
                        min="{{ Carbon::now()->addDay(15)->format('Y-m-d') }}" disabled>
                </div>
            </div>
        </div>
        ...
        <div class="my-5">
            ...
            <div class="sm:ml-10 mt-2">
                <label class=" items-center">
                    <input type="radio" x-model="tipoComprobacionOption" wire:model='tipo_comprobacion'
                        name="tipo_comprobacion" value="vale" title="Este campo no se puede modificar." disabled>
                    <span class="ml-2">Vales</span>
                </label>
                <label class=" items-center ml-6">
                    <input type="radio" x-model="tipoComprobacionOption" wire:model='tipo_comprobacion'
                        name="tipo_comprobacion" value="ficha" title="Este campo no se puede modificar." disabled>
