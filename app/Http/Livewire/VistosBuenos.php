...
class VistosBuenos extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->categoria == 0) {
            $requerimientos = $adquisiciones->union($solicitudes)->orderBy('id')->paginate(5, pageName: 'pendientes');

        } else if ($this->categoria == 1) {

            $requerimientos = $adquisiciones->orderBy('id')->paginate(5, pageName: 'pendientes');

        } else if ($this->categoria == 2) {
            $requerimientos = $solicitudes->orderBy('id')->paginate(5, pageName: 'pendientes');
        }

        if ($this->categoriaVobo == 0) {
            $juntasvobo = $adquisicionesVistosBuenos->union($solicitudesVistosBuenos)->orderBy('id')->paginate(5, pageName: 'vobo');
