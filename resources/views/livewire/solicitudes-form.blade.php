<div class="mt-6">
  ...
  <form wire:submit.prevent="save">
    <div>
      ...
      <div class="overflow-x-auto mt-6" wire:poll x-data="{ elementos: @entangle('recursos').defer, id_rubro: '{{ $id_rubro }}', id_rubro_especial: '{{ $id_rubro_especial }}', monto_total: '{{ $monto_total }}',monto_sumado: @entangle('monto_sumado').defer}">
              ...
              <tr class="border border-b-gray-200 border-transparent">
                ...
                <th>
                  <button type="button" @click='$wire.emit("openModal", "solicitud-recurso-modal",  
                      { _id: elemento._id, concepto: elemento.concepto, importe: elemento.importe, justificacionS: elemento.justificacionS, finicial: elemento.finicial, ffinal: elemento.ffinal, id_rubro: id_rubro, id_rubro_especial: id_rubro_especial, monto_total: monto_total, monto_sumado: monto_sumado })' class="btn-tablas">
      ...
      @if ($id_rubro_especial !== '3')
      <div class="mt-4">
        <input type="checkbox" id="comprobacion" name="comprobacion" wire:model='checkMeobligo' class="mr-1">
        ...
        @error('checkMeobligo') <span class=" text-rojo">{{ $message }}</span> @enderror
      </div>
      @endif
      <div class="mt-4">
        <input type="checkbox" id="aviso_privacidad" name="aviso_privacidad" wire:model='checkAceptoPrivacidad' class="mr-1">
        <label for="aviso_privacidad">Acepto aviso de privacidad simplificada de la UAEMEX</label>
        @error('checkAceptoPrivacidad') <span class=" text-rojo">{{ $message }}</span> @enderror
      </div>
      ...
      <div class="sm:text-right text-center mt-5">
        <button type="submit" wire:click="save()" class="btn-success sm:w-auto w-3/4">Guardar</button>
